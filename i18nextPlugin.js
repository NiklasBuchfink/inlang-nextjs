  ...
  "../../../node_modules/parsimmon/src/parsimmon.js"(exports, module) {
      ...
      if (isBuffer(input)) {
        ...
        lines = map(function (byteLine) {
          return byteLine.length <= 4
            ? byteLine.join(" ")
            : byteLine.slice(0, 4).join(" ") +
                "  " +
                byteLine.slice(4).join(" ");
        }, byteLines);
    ...
    _.map = function (fn) {
      ...
      return Parsimmon2(function (input, i) {
        ...
        return mergeReplies(
          makeSuccess(result.index, fn(result.value)),
          result
        );
      });
    };
    ...
    function unflatten2(target, opts) {
      ...
      target = Object.keys(target).reduce(function (result2, key) {
        ...
        if (!isObject || isEmpty(target[key])) {
          ...
          return addKeys(key, result2, flatten2(target[key], opts));
        }
      }, {});
...
var createParser = (settings) => {
  return import_parsimmon.default.createLanguage({
    ...
    entry: (r) => {
      return import_parsimmon.default
        .alt(r.FunctionCall, import_parsimmon.default.any)
        .many()
        .map((matches) => {
    ...
    doubleQuotedString: () => {
      return import_parsimmon.default
        .string('"')
        .then(import_parsimmon.default.regex(/[^"]*/))
        .skip(import_parsimmon.default.string('"'));
    },
    ...
    FunctionCall: function (r) {
      return import_parsimmon.default.seqMap(
        ...
        (_, __, ___, start, messageId, end, rest) => {
          const namespaceParser = r.comma
            .then(import_parsimmon.default.string("{"))
            .trim(r.whitespace)
            .then(r.nsValue)
            .skip(import_parsimmon.default.string("}"))
            .skip(import_parsimmon.default.regex(/[^)]*/))
            .trim(r.whitespace);
...
var plugin = {
  ...
  addAppSpecificApi: ({ settings }) => ideExtensionConfig(settings),
};
...
async function getFileToParse(path, languageTag, nodeishFs) {
  ...
  try {
    ...
    const flattenedMessages = NESTED[pathWithLanguage]
      ? (0, import_flat.flatten)(JSON.parse(file), {
  ...
  if (typeof args.settings.pathPattern === "object") {
    ...
    for (const message of args.messages) {
      for (const variant of message.variants) {
        const prefix = message.id.includes(":")
          ? message.id.split(":")[0]
    ...
    for (const [languageTag, value] of Object.entries(storage)) {
      const pathWithLanguage = args.settings.pathPattern.replace(
        "{languageTag}",
        languageTag
      );
      try {
...
function serializeFile(
  ...
  return (
    replaceAll(JSON.stringify(result, void 0, space), "u002E", ".") +
    (endsWithNewLine ? "\n" : "")
  );
}
function serializePattern(pattern, variableReferencePattern) {
  ...
  for (const element of pattern) {
    switch (element.type) {
      ...
      default:
        throw new Error(
          `Unknown message pattern element of type: ${element?.type}`
        );
    }
  }
  return result.join("");
  ...
  for (const path of pathArray) {
    ...
    for (const filePath of parentDirectory) {
      const fileExists = await Promise.resolve(
        args.nodeishFs
          .readFile(
            path.replace("{languageTag}", filePath.replace(".json", ""))
          )
          .then(() => true)
          .catch(() => false)
      );
